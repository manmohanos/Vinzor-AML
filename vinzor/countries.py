"""Country codes, as a person reads them.

Reference data, nothing more: ISO 3166-1 alpha-2 to the common English name.
It exists because a screening comparison that puts ``SG`` beside ``cn`` in
front of a compliance officer is not a comparison, it is a puzzle — and the
codes arrive in whatever case the source used.

Display only. Nothing here decides anything, and ``name_of`` falls back to the
code itself rather than guessing, so an unknown or newly-issued code degrades
to what the record actually says.
"""
...
def name_of(code: str) -> str:
    """The country's name, or the code itself if it is not one we know.

    Falling back to the code is deliberate. Guessing at an unrecognised code,
    or blanking it, would both be worse than showing what the record says.
    """
    text = (code or "").strip()
    if len(text) != 2 or not text.isalpha():
        return text
    return COUNTRIES.get(text.upper(), text.upper())
